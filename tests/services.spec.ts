...
test.describe('Unosquare.com Services Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('https://www.unosquare.com/services/');
  ...
  test('Search box works if present', async ({ page }) => {
    ...
    for (const selector of searchSelectors) {
      if (await page.$(selector)) {
        ...
        await randomDelay();
        await expect(page).not.toHaveURL('https://www.unosquare.com/services/');
        break;
      }
    }
